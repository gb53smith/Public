/*
   ...
   Version 1.3 - Use RTCZero.h library for sleep
...
void loop()
{
  ...
  float tempDelta = abs(temperature - lastTemperature);
#ifdef DEBUG
  tempDelta = 100;  //Alway send temperature in DEBUG mode
#endif  
  if ( tempDelta > 0.1)
  {
  ...
  // Sleep using Real Time Clock
  resetAlarm();  // Reset alarm before returning to sleep 
  zerortc.standbyMode();    // Sleep until next alarm match
   // Restoring Radio
  writeRegister(0x01, rfmOpMode); //Retore
#ifdef DEBUG  
  if (alarmFlag) // Flash LED when waking up from sleep.
  {
...
#endif 
  //Not working from MySensors.h
  //SerialUSB.println(sleep(sleepTime)); 
}
